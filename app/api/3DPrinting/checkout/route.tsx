  ...
  try {
    ...
    if (!id) {
      return createErrorResponse(
        "Use must be logged in to place an order",
        201
      );
    }
      ...
      html: `<h2>Dear ${user.firstName} ${
        ...
        (item: any) => {
          return `<h4>${item.name}</h4>
                    <p>Size: ${item.size}</p>
                    <p>
                        Colour: ${item.colour[0]?.toUpperCase()}${item.colour?.substring(
            1
          )}
                    </p>
                    ${
                      item.base
                        ? `<p>
                            Base: ${item.base[0]?.toUpperCase()}${item.base?.substring(
                            1
                          )}
                        </p>`
                        : ""
                    ...
                    <p>
                        R${item.price} <span style="margin-left: 10px;">Qty: ${
            item.qty
          }</span>
                    </p>`;
        }
            ...
            <p>${body.shipping[0].toUpperCase()}${body.shipping.substring(
        1
      )}</p>
            ${
              body.shipping === "deliver"
