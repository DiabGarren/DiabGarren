...
export default function ItemPage({ params }: { params: { id: string } }) {
    ...
    const addToCart = async () => {
        const res = await fetch(
            process.env.NEXT_PUBLIC_API_URL + "/3DPrinting/cart",
            {
                method: "POST",
        ...
        >
            <div className="md:w-[90%]">
                ...
                <p className="mt-[10px] hidden md:block">
                    All sizes are in the format: <br />{" "}
                    <b>Length x Width x Height</b>
                </p>
            </div>

            <div>
                <h1 className="text-[25px] xsm:text-[30px] font-[700]">
                    {item?.name}
                </h1>
                <h2>
                    Size (cm){" "}
